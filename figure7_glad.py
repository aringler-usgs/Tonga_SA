...
plt.xlabel('Deviation from GLAD25 (%)')
plt.ylim((min(range(len(stas)))-0.5, max(range(len(stas)))+0.5))
plt.subplot(1,2,2)
plt.plot(upos,range(len(stas)), '.', 'C1', markersize=20, label='Calculated')
plt.plot(zrats, range(len(stas)), '.', 'C2', markersize=20, label='data')
plt.legend()
...
# # plt.ylim(0.,100.)
# # #plt.tight_layout()
plt.savefig('Figure7_withglad.png', format='PNG', dpi=400)
plt.savefig('Figure7_withglad.pdf', format='PDF', dpi=400)
# f.close()
